...
[Technology stack images/descriptions]
...
3. Build and run the project directly: `mvn spring-boot:run`
4. The default profile uses a local H2 database at `./data/bankingportal`, enables the H2 console at `/h2-console`, and keeps email plus geolocation integrations disabled unless you opt in.
5. If you want MySQL instead, start the local dependencies with `docker compose -f docker/docker-compose.yml up -d` and set `DB_URL=jdbc:mysql://localhost:3306/bankingapp`, `DB_USERNAME=bankinguser`, and `DB_PASSWORD=bankingpass` before starting the app.
